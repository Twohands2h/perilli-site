...
function Lightbox({
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center" onClick={onClose}>
            <button onClick={onClose} className="absolute top-4 right-4 z-10 p-2 text-white/70 hover:text-white transition-colors" aria-label="Close"><X size={28} /></button>
            ...
            <div className="relative w-[90vw] h-[85vh] max-w-6xl" onClick={(e) => e.stopPropagation()}>
                <Image src={images[index].src} alt={images[index].alt} fill className="object-contain" sizes="90vw" priority />
            </div>
...
function useLightbox() {
    ...
    const open = useCallback((idx: number) => { setStartIndex(idx); setIsOpen(true); }, []);
...
function LightboxImage({ src, alt, fill: fillProp, className, sizes, priority, lightbox }: {
    ...
    return (
        <div className="cursor-zoom-in" onClick={() => lightbox.open(idx)}>
            <SafeImage src={src} alt={alt} fill={fillProp} className={className} sizes={sizes} priority={priority} />
        </div>
    );
}
...
function useVideoManager() {
    ...
    const registerIframe = useCallback((el: HTMLIFrameElement | null) => { if (el) iframesRef.current.add(el); }, []);
    const registerVideo = useCallback((el: HTMLVideoElement | null) => { if (el) videosRef.current.add(el); }, []);

    const stopOthers = useCallback((currentEl: HTMLElement) => {
        videosRef.current.forEach((v) => { if (v !== currentEl && !v.paused) v.pause(); });
...
function BlockRenderer({ block, locale, vm, lightbox }: {
    block: ContentBlock; locale: string;
    vm: ReturnType<typeof useVideoManager>; lightbox: ReturnType<typeof useLightbox>;
}) {
            ...
            return (
                <div className="section-container">
                    <div className="max-w-3xl">
                        <div className="text-text-secondary text-sm md:text-base leading-relaxed [&_a]:text-accent [&_a]:underline [&_a]:underline-offset-2"
            ...
            return (
                <div className={block.fullWidth ? 'w-full' : 'section-container'}>
                    <div className={`relative ${block.fullWidth ? 'aspect-[21/9]' : 'aspect-video'} overflow-hidden ${block.fullWidth ? '' : 'rounded-lg'} bg-surface`}>
                        <LightboxImage src={block.src} alt={isIt ? block.alt.it : block.alt.en} fill className="object-cover" sizes="100vw" lightbox={lightbox} />
                    </div>
                </div>
            );
        case 'gallery': {
            ...
            return (
                <div className="section-container">
                    ...
                    <div className="relative aspect-video rounded-lg overflow-hidden bg-black">
                        <iframe ref={vm.registerIframe} src={iframeSrc} className="absolute inset-0 w-full h-full" style={{ border: 0 }}
                            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; fullscreen" allowFullScreen title={block.title || 'Video'} />
                    </div>
                </div>
            );
        }
        ...
        case 'video-loop':
            return (
                <div className="section-container">
                    {block.title && <p className="text-xs font-semibold uppercase tracking-wider text-text-muted mb-3">{block.title}</p>}
                    <div className="relative aspect-video rounded-lg overflow-hidden bg-black">
                        <video autoPlay muted loop playsInline preload="metadata" poster={block.poster} className="w-full h-full object-cover">
                            <source src={block.url} type="video/mp4" />
                        </video>
                    </div>
                </div>
    ...
    return (
        <article>
            ...
            {(project.videoEmbed || project.videoUrl) && (
                <VideoObjectSchema name={title} description={isIt ? project.briefingIt : project.briefingEn}
                    thumbnailUrl={`https://pieroperilli.com${project.thumbnail}`} uploadDate={project.year}
                    contentUrl={project.videoUrl}
                    embedUrl={project.videoEmbed ? (project.videoType === 'youtube' ? `https://www.youtube.com/embed/${project.videoEmbed}` : `https://player.vimeo.com/video/${project.videoEmbed}`) : undefined} />
            )}
            {project.contentBlocks?.filter((b): b is Extract<ContentBlock, { type: 'video' }> => b.type === 'video').map((block, i) => (
                <VideoObjectSchema key={`video-schema-${i}`} name={block.title || `${title} — Video ${i + 1}`}
                    description={isIt ? project.briefingIt : project.briefingEn} thumbnailUrl={`https://pieroperilli.com${project.thumbnail}`}
                    uploadDate={project.year}
                    embedUrl={block.platform === 'youtube' ? `https://www.youtube.com/embed/${block.embedId}` : `https://player.vimeo.com/video/${block.embedId}`} />
            ))}

            {/* Sticky breadcrumb */}
            <div className="sticky top-16 md:top-20 z-40 bg-background/90 backdrop-blur-md border-b border-border/30">
                <div className="section-container py-3">
                    <Link href={isIt ? '/portfolio' : '/en/portfolio'}
            ...
            <section className="pt-8 pb-6 md:pt-12 md:pb-8 lg:pt-16 lg:pb-12">
                <div className="section-container">
                    <AnimateOnScroll>
                        ...
                        <h1 className="font-bold text-text-primary mb-4" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', lineHeight: '1.08', letterSpacing: '-0.025em' }}>
                            {title}
                        </h1>
                        ...
                        {sections.map((s, i) => (
                            <AnimateOnScroll key={i} delay={i * 60}>
                                <div className="py-6 md:py-8 border-t border-border">
                                    <h2 className="text-xs font-bold uppercase tracking-[0.15em] text-accent mb-4">{isIt ? s.labelIt : s.labelEn}</h2>
                                    <p className="text-text-secondary text-sm md:text-base leading-relaxed" dangerouslySetInnerHTML={{ __html: isIt ? s.contentIt : s.contentEn }} />
                                </div>
                            </AnimateOnScroll>
                        ))}
                    </div>
                                ...
                                <AnimateOnScroll>
                                    <div className="relative aspect-video rounded-lg overflow-hidden bg-black">
                                        {project.videoType === 'youtube' && project.videoEmbed && (
                                            <iframe src={`https://www.youtube.com/embed/${project.videoEmbed}?rel=0&modestbranding=1`}
                                                className="absolute inset-0 w-full h-full" style={{ border: 0 }}
                                                allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen title={title} />
                                        )}
                                        {project.videoType === 'vimeo' && project.videoEmbed && (
                                            <iframe src={`https://player.vimeo.com/video/${project.videoEmbed}?title=0&byline=0&portrait=0`}
                                                className="absolute inset-0 w-full h-full" style={{ border: 0 }}
                ...
                <div className="section-container pt-8">
                    <div className="flex flex-wrap gap-3 mb-6">
                        {project.tools.map((tool) => (
                            <span key={tool} className="px-3 py-1.5 text-xs font-semibold uppercase tracking-wider bg-surface border border-border rounded-sm text-text-muted">{tool}</span>
                        ))}
                    </div>
                    {project.imdbLink && (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-6">
                            {related.map((rel, i) => (
                                <AnimateOnScroll key={rel.slug} delay={i * 100}>
                                    <Link href={`${isIt ? '' : '/en'}/portfolio/${rel.slug}`} className="group block relative aspect-[16/10] rounded-lg overflow-hidden bg-surface">
                                        {rel.thumbnailVideo ? (
                                            <VideoThumbnail src={rel.thumbnail} videoSrc={rel.thumbnailVideo} alt={isIt ? rel.thumbnailAlt.it : rel.thumbnailAlt.en}
                                                fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 33vw" />
                                        ) : (
                                            <SafeImage src={rel.thumbnail} alt={isIt ? rel.thumbnailAlt.it : rel.thumbnailAlt.en}
                                                fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 33vw" />
                                        )}
                                        ...
                                        <div className="absolute bottom-0 left-0 right-0 p-4 md:p-5">
                                            <span className="text-[10px] font-semibold uppercase tracking-wider text-accent mb-1.5 block">{isIt ? rel.categoryLabel.it : rel.categoryLabel.en}</span>
                                            <h3 className="text-sm md:text-base font-bold text-text-primary">{isIt ? rel.titleIt : rel.titleEn}</h3>
                                        </div>
                                    </Link>
                                </AnimateOnScroll>
                            ))}
                ...
                <div className="relative section-container text-center">
                    <AnimateOnScroll>
                        ...
                        <p className="text-text-secondary max-w-xl mx-auto mb-10 text-sm md:text-base">
                            {isIt ? 'Raccontami di cosa hai bisogno. Leggo personalmente ogni richiesta e rispondo in tempi brevi.' : 'Tell me what you need. I personally read every request and respond promptly.'}
                        </p>
                    </AnimateOnScroll>
            ...
            {/* Lightbox */}
            {lightbox.isOpen && <Lightbox images={lightbox.images} startIndex={lightbox.startIndex} onClose={lightbox.close} />}
        </article>
    );
}
