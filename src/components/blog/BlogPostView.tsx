            ...
            <section className="pt-8 pb-6 md:pt-12 md:pb-8 lg:pt-16 lg:pb-12">
                <div className="section-container">
                    <AnimateOnScroll>
                        ...
                        <h1 className="font-bold text-text-primary mb-4 max-w-3xl"
                            style={{ fontSize: 'clamp(1.75rem, 4.5vw, 3rem)', lineHeight: '1.1', letterSpacing: '-0.02em' }}>
                            {title}
                        </h1>
                ...
                <div className="section-container">
                    <AnimateOnScroll>
                        <div className="relative aspect-[21/9] rounded-lg overflow-hidden bg-surface">
                            <SafeImage src={post.thumbnail} alt={isIt ? post.thumbnailAlt.it : post.thumbnailAlt.en}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            {related.map((rel, i) => (
                                <AnimateOnScroll key={rel.slug} delay={i * 80}>
                                    <Link href={`${isIt ? '' : '/en'}/blog/${getPostSlug(rel, locale)}`} className="group block">
                                        <div className="relative aspect-[16/9] rounded-lg overflow-hidden bg-surface mb-4">
                                            <SafeImage src={rel.thumbnail} alt={isIt ? rel.thumbnailAlt.it : rel.thumbnailAlt.en}
                                                fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 50vw" />
                                        </div>
